...
const JoystickComponents = (props) => {
  ...
  const joystickBaseMaterial = React.useMemo(
    () => new THREE__namespace.MeshBasicMaterial({ transparent: true, opacity: 0.3 }),
    []
  );
  ...
  const joystickHandleMaterial = React.useMemo(
    () => new THREE__namespace.MeshBasicMaterial({ transparent: true, opacity: 0.7 }),
    []
  );
...
const ButtonComponents = ({
  ...
  const buttonBase1Material = React.useMemo(
  ...
  const buttonBase2Material = React.useMemo(
    () => new THREE__namespace.MeshBasicMaterial({
      transparent: true,
      opacity: 0.3,
      color: "white"
...
const Ecctrl = ({
  ...
  const characterModelIndicator = React.useMemo(
  ...
  const bodyBalanceVecOnX = React.useMemo(
    () => new THREE__namespace.Vector3(),
    []
  );
  const bodyFacingVecOnY = React.useMemo(
    () => new THREE__namespace.Vector3(),
    []
  );
  const bodyBalanceVecOnZ = React.useMemo(
    () => new THREE__namespace.Vector3(),
    []
  );
  ...
  const bodyContactForce = React.useMemo(
    () => new THREE__namespace.Vector3(),
    []
  );
  const slopeRayOriginUpdatePosition = React.useMemo(
    () => new THREE__namespace.Vector3(),
    []
  );
  const camBasedMoveCrossVecOnY = React.useMemo(
    () => new THREE__namespace.Vector3(),
    []
  );
  ...
  const gamepadJoystickVec2 = React.useMemo(
  ...
  const mergedKeys = React.useMemo(
  ...
  const handleSticks = (axes) => {
    if (Math.abs(axes[0]) > 0 || Math.abs(axes[1]) > 0) {
      ...
      gamepadJoystickDis = Math.min(
        Math.sqrt(
          Math.pow(gamepadJoystickVec2.x, 2) + Math.pow(gamepadJoystickVec2.y, 2)
        ),
        1
      );
  ...
  const standingForcePoint = React.useMemo(
    () => new THREE__namespace.Vector3(),
    []
  );
  const movingObjectDragForce = React.useMemo(
    () => new THREE__namespace.Vector3(),
    []
  );
  const movingObjectVelocity = React.useMemo(
    () => new THREE__namespace.Vector3(),
    []
  );
  ...
  const distanceFromCharacterToObject = React.useMemo(
    () => new THREE__namespace.Vector3(),
    []
  );
  const objectAngvelToLinvel = React.useMemo(
    () => new THREE__namespace.Vector3(),
    []
  );
  ...
  const characterMassForce = React.useMemo(
    () => new THREE__namespace.Vector3(),
    []
  );
  ...
  const actualSlopeNormalVec = React.useMemo(
    () => new THREE__namespace.Vector3(),
    []
  );
  const floorNormal = React.useMemo(
  ...
  fiber.useFrame((state, delta) => {
    ...
    if (slopeRayHit) {
      actualSlopeNormal = (_c = slopeRayHit.collider.castRayAndGetNormal(
        slopeRayCast,
        slopeRayLength,
        false
      )) == null ? void 0 : _c.normal;
